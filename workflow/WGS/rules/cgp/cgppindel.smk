...
rule PI_UM: 
    input:
        vcfs=expand("{project}/{genome_version}/analysis/pindel_normal/log/{normal}_pindel_NC.log",normal = paired_samples,project = project,genome_version = genome_version)
    output:
        # expand('{project}/{genome_version}/analysis/normalPanel/pindel_{sample}.gff3.gz',sample = ['merge'],project = project,genome_version = genome_version)
        '{project}/{genome_version}/analysis/normalPanel/pindel_{sample}.gff3.gz'
    threads: 20
    params:
        ref=config['resources'][genome_version]['REFFA'],
        gff3='{project}/{genome_version}/analysis/normalPanel/pindel_merge'
    singularity:
        config['singularity']['cgppindel']['sif']
        # '/public/ClinicalExam/lj_sih/softwares/pindel.sif'
    shell:
...
rule cgppindel_filter_somatic:
    input:
        vcf='{project}/{genome_version}/logs/paired/cgppindel_{sample}.log'
    output:
