...
parser = argparse.ArgumentParser(description="Converts VarScan2 somatic vcf to native format and vice-versa.\nInput is automatically detected")
...
def printVcfHeader():
    print("##fileformat=VCFv4.1\n"
          ...
          "#CHROM\tPOS\tID\tREF\tALT\tQUAL\tFILTER\tINFO\t"+sample_name+"_NC\t"+sample_name+"_T")
...
def makeVcfRecord(nativeIp):
    ...
    print("%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s" % (chrom, pos, id, ref, alt, qual, filter, info, normal_format, tumor_format))
